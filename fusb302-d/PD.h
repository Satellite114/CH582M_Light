...
#define PD_isDataMessage(__msg__) (PD_HeaderWord_getNumberOfDataObjects((__msg__)->Header.Word) > 0)
	...
	// void PD_newMessage( PD_Message_t * message, uint8_t numberOfDataObjects, uint8_t messageId, uint16_t powerRole, uint16_t specRev, uint16_t dataRole, uint16_t commandCode, PD_DataObject_t * dataObjects )
	// {
		...
		PD_DataObject_t *dataObjects)
	{
		...
		message->Header.Word = 0;
		message->Header.Word |= PD_HeaderWord_setNumberOfDataObjectsBits(numberOfDataObjects);
		message->Header.Word |= PD_HeaderWord_setMessageIdBits(messageId);
		message->Header.Word |= powerRole | specRev | dataRole | commandCode;
