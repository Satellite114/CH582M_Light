...
i2c_transfer_data(addr, len, buf);
...
static FUSB302_D_Error_t FUSB302_D_ReadReg(FUSB302_D_t *fusb, uint8_t registerAddress, uint8_t *buf, uint16_t len)
{
    ...
    uint16_t addr = fusb->Addr << 1; // STM32 HAL 使用 7-bit 地址左移 1 位


    i2c_transfer_data(addr, 1, &registerAddress);

 
    i2c_recv_data(addr, len, buf);
    // 可选：打印读取的数据
    // printf("FUSB302 read reg 0x%02X = ", registerAddress);
...
static FUSB302_D_Error_t FUSB302_D_SequentialWrite(FUSB302_D_t *fusb, uint8_t *buf, uint16_t len, uint32_t XferOptions)
{
    ...
    // HAL_StatusTypeDef status = HAL_I2C_Master_Transmit(fusb->Hi2c, addr, buf, len, XferOptions);
    ...
    // {
    //     printf("slave didn't acknowledge\n");
    //     return FUSB302_D_ERROR;
    // }
...
FUSB302_D_Error_t FUSB302_D_Init(FUSB302_D_t *fusb, uint16_t i2cAddr)
{
...
FUSB302_D_Error_t FUSB302_D_Read(FUSB302_D_t *fusb, FUSB302_D_Register_t reg, uint8_t *data)
{
    ...
    // HAL_I2C_Master_Receive(&hi2c1, 0x22 << 1, &id, 1, 100)
...
void FUSB302_D_Test(uint16_t i2cAddr)
{
    FUSB302_D_t fusb;
    //	FUSB302_D_Error_t error;

    FUSB302_D_Init(&fusb, i2cAddr);
